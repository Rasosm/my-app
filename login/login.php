...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_GET['logout'])) {
        unset($_SESSION['user']);
        header('Location: http://localhost/js-002/my-app/login/login.php');
        die;
    }
    ...
    foreach($users as $user) {
        if ($user['name'] == $_POST['name']) {
            if ($user['psw'] == md5($_POST['psw'])) {
                $_SESSION['user'] = $user;
                header('Location: http://localhost/js-002/my-app/login/member.php');
                die;
            }
        }
    }
    header('Location: http://localhost/js-002/my-app/login/login.php?error');
    die;
}
